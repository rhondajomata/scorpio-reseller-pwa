    ...
    <body>
        <div class="container">
            ...
            <div id="resultContainer">
                <div class="price-box">
                    <div class="price-title">💰 Smart Pricing Recommendations</div>
                    <div id="priceText">Calculating...</div>
                </div>
        ...
        <script>
            ...
            async function runAIScan() {
                ...
                if (shirtInput.files.length === 0 || tagInput.files.length === 0) {
                    alert("Please provide both image captures.");
                    return;
                }
...
Yes, exactly! But you shouldn't type it all out line-by-line. Because the code block you pasted has lost its correct Python indentation (spacing), copying it directly will cause another error.
To avoid any formatting headaches, I have cleanly spaced everything out for you below.
## 🛠️ What to do:

   1. Copy the clean code block below.
   2. Replace everything inside your local Notepad file with it and save.
   3. Replace everything inside your GitHub app.py file with it and click Commit changes.

import osfrom fastapi import FastAPI, UploadFile, Filefrom fastapi.middleware.cors import CORSMiddlewarefrom fastapi.responses import HTMLResponsefrom google import genaifrom google.genai import types
app = FastAPI()
...
@app.get("/", response_class=HTMLResponse)async def read_index():
    return """
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
if (data.success) {
priceText.innerHTML = data.pricing.replace(/\n/g, "
");
...
alert("Server communication loss.");
console.error(err);
} finally {
btn.disabled = false;
...
response_listing = client.models.generate_content(
model='gemini-2.5-flash',
contents=[image_shirt, image_tag, prompt_listing]
)
# 2. Generate Pricing Suggestions
prompt_pricing = (
"Analyze the brand, style, material, and condition of this item from the images. "
"Provide a realistic marketplace resale valuation. Output exactly three lines format:\n"
"• ⏱️ Fast Flip (Low Entry): $XX\n"
"• 🎯 Sweet Spot (Fair Value): $XX\n"
"• 💎 Max Profit (Patient Seller): $XX\n\n"
"Add a very short sentence at the end explaining why (e.g., based on brand popularity or material quality)."
)
response_pricing = client.models.generate_content(
model='gemini-2.5-flash',
contents=[image_shirt, image_tag, prompt_pricing]
)
return {
"success": True,
"listing": response_listing.text,
"pricing": response_pricing.text
}
except Exception as e:
return {"success": False, "error": str(e)}
except Exception as base_e:
return {"success": False, "error": str(base_e)}
